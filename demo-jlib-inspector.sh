...
if ! command -v mvn &> /dev/null; then
    echo -e "${RED}Error: maven is required but not installed.${NC}"
...
AGENT_PATH="agent/target/jlib-inspector-agent-1.0-SNAPSHOT.jar"
...
java -cp "agent/target/classes" io.github.brunoborges.jlib.server.JLibServer 8080 > /dev/null 2>&1 &
SERVER_PID=$!
...
echo -e "${GRAY}  -javaagent:jlib-inspector-agent.jar                    # Local only${NC}"
echo -e "${GRAY}  -javaagent:jlib-inspector-agent.jar=server:8080        # Report to localhost:8080${NC}"
echo -e "${GRAY}  -javaagent:jlib-inspector-agent.jar=server:remote:9000 # Report to remote:9000${NC}"
